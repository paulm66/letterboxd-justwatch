...
# get watchlist
watchlist_titles = []
watchlist_pages = get_page_count(letterboxd_username)
#watchlist_years = []
...
#todo:replace this with https://github.com/Electronic-Mango/simple-justwatch-python-api

#Make all this a function and call it once per letterboxd entry
#add code to figure out if its the right film? Maybe check year too?
from simplejustwatchapi.justwatch import search
#results = search("Dune", "GB")
#object access semantics make it simple
#default is to return 4 nearest matches so should check that the title matches my title and check the other 3 if not
results[0].title
#next grab offers and loop
all_offers = results[0].offers
for offer in all_offers:
	#grab offer name and check if its in the list provided
	results[0].offers[0].technical_name
	#if its in the provider list grab the price info 
	results[0].offers[0].monetization_type
	results[0].offers[0].price_value
	#add to list then at the end print it

result = {}
for movie in watchlist_titles:
	results = search(movie, just_watch_region)
	#check the name matches here?
	all_offers = results[0].offers
	for offer in all_offers:
		#build tuple of provider to offer monetization and value
		is_it_free = offer.monetization_type
		cost = offer.price_value
		#add films that cost money on the services and ones that dont
		result['Free Services'].append(movie+' ('+str(movies_found['items'][0]['original_release_year'])+')')
